...
//
//   docs -> skills   an emgr documentation page that no skill claims fails
...
//                    the image-resizer checkout fails the gate. That is the
...
// ---------------------------------------------------------------------------
//
// 1. THE SENTINEL IS NOT `AGENTS.md`. Both exemplars test for an `AGENTS.md`
//    to decide "does this look like a real checkout". **image-resizer has no
//    `AGENTS.md` and no `CLAUDE.md`** — its agent instructions live in
//    `.roo/rules/*.md` instead. So this checks `Cargo.toml` AND that it
//    declares `name = "emgr"`. The manifest alone would match any Rust repo
//    on the machine; the package name is what makes the check identifying.
//
// 2. THE REPOSITORY IS `image-resizer`; THE PROJECT IS `emgr`. They are not
//    the same word and nothing renames one to the other. The variable, the
//    env var and the baseline keys all say `emgr` (the thing the skills
//    describe); the default path and the CI checkout say `image-resizer`
//    (the thing you clone).
//
...
//    replaced there. Nothing of the sort exists in image-resizer either.
//    What DOES exist, machine-enumerable and genuinely one-file-per-topic:
//
//      docs/**/*.md          the mkdocs tree (mkdocs.yml's own `nav`)
//      adr/*.md              one architecture decision per file
//      helm/*/Chart.yaml     one chart per deployable shape
//      .roo/rules/*.md       the repo's agent rules — this repo's closest
//                            thing to an AGENTS.md, and directly this
//                            skills repo's business
//
...
//    claim on `docs/architecture` covers its pages, a claim on bare `docs`
//    is not a claim at all. That is the same rule vsms arrived at by hand
//    (it removed blanket `docs/runbooks` and `docs/design` entries for
//    exactly this reason), expressed once instead of per-surface.
//
// Usage:  node tools/verify-coverage.mjs [path-to-image-resizer-checkout]
//         EMGR_REPO=/path/to/image-resizer node tools/verify-coverage.mjs
//
...
const emgr = resolve(
  process.argv[2] ?? process.env.EMGR_REPO ?? "../image-resizer",
);

// See adaptation note 1: `AGENTS.md` does not exist in this source repo, so
// the sentinel is the package manifest plus the package name it declares.
const manifest = join(emgr, "Cargo.toml");
if (
  !existsSync(manifest) ||
  !/^name\s*=\s*"emgr"\s*$/m.test(readFileSync(manifest, "utf8"))
) {
  console.error(
    `verify-coverage: ${emgr} does not look like an image-resizer checkout ` +
      `(no Cargo.toml declaring \`name = "emgr"\`).\n` +
      `Pass the path: node tools/verify-coverage.mjs /path/to/image-resizer`,
  );
  process.exit(2);
}
...
// ------------------------------------------------------------------- baseline
//
// A skill is true of *an* emgr, not of emgr. The project does cut
...
if (baseline.emgrRef) {
  try {
    ...
    if (head.startsWith(baseline.emgrRef) || baseline.emgrRef.startsWith(head)) {
      drift =
        `baseline: emgr ${headShort} (${headDate}) — exactly the tree these ` +
      ...
      drift =
        `baseline: these skills were verified against emgr ` +
        `${baseline.emgrRef.slice(0, 8)} (${baseline.verifiedAt ?? "undated"}); ` +
    ...
    drift =
      `baseline: ${emgr} is not a git checkout — cannot report drift from ` +
      `${baseline.emgrRef.slice(0, 8)}`;
  }
} else {
  note(
    `coverage.json has no "baseline" block. Every published skill set names the ` +
      `emgr commit it was verified against — see VERSIONING.md.`,
  );
}
...
for (const dir of skillDirs) {
  ...
  // Rule 1 of VERSIONING.md: every skill names the emgr it was verified
  // against. Enforced rather than asked for, because the whole point is that
  // it must never be the line someone forgets.
  const stamp =
    /Verified against emgr `([0-9a-f]{7,40})` \((\d{4}-\d{2}-\d{2})\)/.exec(src);
  if (!stamp) {
    note(
      `skills/${dir}/SKILL.md carries no version stamp. Add, under the title:\n` +
        "        > **Verified against emgr `<sha>` (<YYYY-MM-DD>).** …  — see VERSIONING.md",
    );
  } else if (baseline.emgrRef && !baseline.emgrRef.startsWith(stamp[1])) {
    // A stamp NEWER than the baseline is correct and expected: one skill
    // re-verified against a later emgr without re-verifying the others.
    ...
    try {
      execFileSync(
        "git",
        ["-C", emgr, "merge-base", "--is-ancestor", baseline.emgrRef, stamp[1]],
        { stdio: "ignore" },
      );
      descendant = true;
    } catch {
    ...
    if (!descendant) {
      note(
        `skills/${dir}/SKILL.md is stamped emgr ${stamp[1]}, which is not the ` +
          `baseline (${baseline.emgrRef.slice(0, 8)}) and does not contain it. ` +
...
// ------------------------------------------- emgr -> skills (docs covered)

// The four real, enumerable "one file per topic" surfaces. See adaptation
...
const surfaces = [
  ...
  // One architecture decision per file. 0004 is superseded by 0005 and kept
  // for the record; a superseded ADR still needs a skill to say so, which is
  // why it is claimed rather than exempted.
  { root: "adr", match: (p) => p.endsWith(".md") },
  // One chart per deployable shape (`helm/emgr` Deployment, `helm/serverless`
  // Knative). A chart is a thing the system does, and it is the easiest kind
  // of thing to add without anyone writing a briefing for it.
  { root: "helm", match: (p) => p.endsWith("/Chart.yaml") },
  // This repo's agent rules — its closest thing to an AGENTS.md, and squarely
  // this repository's own business.
  { root: ".roo/rules", match: (p) => p.endsWith(".md") },
];
...
for (const surface of surfaces) {
  const abs = join(emgr, surface.root);
  if (!existsSync(abs)) {
    note(
      `${emgr}/${surface.root} does not exist. This gate's emgr -> skills ` +
        `direction has nothing to check under it — either image-resizer ` +
...
const existedAtBaseline = (path) => {
  if (!baseline.emgrRef) return true; // no baseline: the gate already said so
  try {
    execFileSync(
      "git",
      ["-C", emgr, "cat-file", "-e", `${baseline.emgrRef}:${path}`],
      { stdio: "ignore" },
    );
    return true;
  } catch {
...
// surface root — so `docs/architecture` is a claimable directory but bare
// `docs` is not, and `helm/emgr` is but bare `helm` is not. See adaptation
// note 4. A surface-root claim would make every file under it pass regardless
...
for (const doc of docs) {
  if (!covers(doc)) {
    ...
    note(
      inherited
        ? `${doc} was added to image-resizer AFTER the baseline ` +
            `(${baseline.emgrRefShort ?? baseline.emgrRef?.slice(0, 8)}).\n` +
            ...
            `reason.`
        : `${doc} is an emgr documentation page that no skill covers.\n` +
...
for (const e of exempt) {
  if (!docs.includes(e) && !existsSync(join(emgr, e))) {
    note(`coverage.json exempts ${e}, which no longer exists in image-resizer`);
  }
...
if (failures.length > 0) {
  ...
  // runs this against an older image-resizer.
  if (drift) console.error(`\n  ${drift}`);
  console.error(
    `\nThis gate is the docs↔skills parity rule. An emgr feature that ships ` +
...
console.log(
  `verify-coverage: ${skillDirs.length} skills, ` +
    `${claimed.size} emgr paths claimed, ` +
    `${docs.length} documentation pages across ${surfaces.length} surfaces, ` +
    `${exempt.size} exempt — parity against ${emgr}`,
);
